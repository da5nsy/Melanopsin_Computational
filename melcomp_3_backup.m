...
if vw == 0 %no variable weights
    pc_m.variableweights = ones(81,1);
elseif vw == 1 %S + L and max in between
    pc_m.variableweights = T_SSF(:,1)+T_SSF(:,3); 
    [~,t_p_locs] = findpeaks(pc_m.variableweights);
    pc_m.variableweights(t_p_locs(1):t_p_locs(2)) = max(pc_m.variableweights);
    %plot(pc.variableweights)
elseif vw == 2 %S+L+I
    pc_m.variableweights = T_SSF(:,1)+T_SSF(:,3)+T_mel(:,1); 
end
...
[pc_m.COEFF, pc_m.SCORE, pc_m.LATENT, pc_m.TSQUARED, pc_m.EXPLAINED] = pca(T_SPD','VariableWeights',pc_m.variableweights);
...
for j=1:nPC
    for i=1:size(cs,1)
        c(i,j) = corr(squeeze(mean(cs(i,:,:),2)),pc_m.SCORE(:,j));
    end
end
c=abs(c);
...
if plt_sca
    ...
    for i=1:size(cs,1)
        for j=1:nPC
            ...
            scatter(squeeze(mean(cs(i,:,:),2)),pc_m.SCORE(:,j),'r.')
            if j == 1
